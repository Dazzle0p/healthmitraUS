...
import { getFranchise, assignModules, getFranchiseKYC, updateFranchiseKYC, verifyFranchiseKYC } from '@/app/actions/franchise';
    ...
    const handleDocUpload = async (docType: 'aadhaar_front' | 'aadhaar_back' | 'pan_card' | 'photo', file: File) => {
        ...
        try {
            ...
            if (result.success && result.data?.url) {
                // Update DB with URL via server action
                await updateFranchiseKYC(id, { [docType === 'aadhaar_front' ? 'aadhaarFront' : docType === 'aadhaar_back' ? 'aadhaarBack' : docType === 'pan_card' ? 'panCard' : 'photo']: result.data.url });
                ...
                <div>
                    ...
                    {onUpload && (
                        <label className="cursor-pointer">
                            <input type="file" accept="image/*,.pdf,.jpg,.jpeg,.png,.doc,.docx" className="hidden" onChange={(e) => {
